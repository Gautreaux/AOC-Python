...
def generateNbyMWindows(
    ...
    transforms = list(tuple(itertools.product(range(m_columns), range(n_rows))))
...
def maxPowerAndPos_variable(
    grid: FuelCellGrid_T, max_size: int = 300
) -> tuple[tuple[int, int], int, int]:
    """Return max power and pos considering all windows"""

    assert max_size > 1

    cache_by_window_size: list[dict[tuple[int, int], int]] = [
        {},
        grid,
    ]
