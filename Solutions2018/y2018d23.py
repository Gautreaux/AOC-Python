...
def y2018d23(inputPath = None):
    ...
    bots_in_range = list(filter(
        lambda x: sum(map(lambda x,y: abs(x-y), x.pos, strongest_nanobot.pos)) <= strongest_nanobot.radius,
        nanobots,
    ))
    ...
    def generateNIntersectionGroups(n: int) -> Generator[tuple[NanoBot,...], None, None]:
