...
def y2018d4(inputPath = None):
    ...
    most_asleep_guard_id, mins_asleep_qty = max(mins_asleep_by_guard_id.items(), key=lambda x: x[1])

    asleep_by_max_guard = Counter(map(lambda x: x[1], filter(lambda x: x[0] == most_asleep_guard_id, all_mins)))

    most_asleep_min, most_asleep_min_qty = max(asleep_by_max_guard.items(), key=lambda x: x[1])
