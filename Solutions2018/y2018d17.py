...
class TileType(IntEnum):
    ...
    def wontDrain(self) -> bool:
        return (
            self == TileType.CLAY
            or self == TileType.TRAPPED_SAND
            or self == TileType.WET_TRAPPED_SAND
        )
...
def y2018d17(inputPath=None):
    ...
    water_tiles_count = sum(
        1
        for _ in filter(
            lambda x: x[1].isWet(),
            filter(lambda x: x[0][1] <= max_y and x[0][1] >= min_y, tiles.items()),
        )
    )
