...
class SensorBeaconPair:
    ...
    def generate_points_where_beacon_is_excluded(
        self, y_test: int = _DEFAULT_Y_TEST
    ) -> Iterator[DiscretePoint2]:
        """Generate the set of points where the beacon cannot be"""

        sensor_dist_to_beacon = self.sensor_position.manhattan_distance(
            self.beacon_position
        )
        sensor_dist_to_y = self.sensor_position.manhattan_distance(
            DiscretePoint2(self.sensor_position.x, y_test)
        )
        ...
        for x in range(
            self.sensor_position.x - x_offset, self.sensor_position.x + x_offset + 1
        ):
    ...
    def get_aabb_where_beacon_is_excluded(
        self, y_test: int = _DEFAULT_Y_TEST
    ) -> Iterator[DiscreteAABBbounds]:
        ...
        if self.beacon_position.y != y_test:
            ...
            yield DiscreteAABBbounds(
                DiscretePoint1(x_min), DiscretePoint1(self.beacon_position.x - 1)
            )
            yield DiscreteAABBbounds(
                DiscretePoint1(self.beacon_position.x + 1), DiscretePoint1(x_max)
            )
...
class Solution_2022_15(SolutionBase):
    ...
    def __post_init__(self):
        ...
        for line in self.input_str_list(include_empty_lines=False):
            ...
            self.sensors_and_beacons.append(
                SensorBeaconPair(DiscretePoint2(x_s, y_s), DiscretePoint2(b_x, b_y))
            )
    ...
    def _generate_candidate_distress_coordinates(
        self,
        y_test: int = _DEFAULT_Y_TEST,
        x_bounds: DiscreteAABBbounds = DiscreteAABBbounds(
            DiscretePoint1(0), DiscretePoint1(4000000)
        ),
    ) -> Iterator[DiscretePoint2]:
