...
class Solution_2022_08(SolutionBase):
    ...
    def generate_neighbors(
        self, point: DiscretePoint2
    ) -> tuple[Point2_Gen_T, Point2_Gen_T, Point2_Gen_T, Point2_Gen_T]:
        ...
        return (
            ...
            map(
                point_factory, range(point.x + 1, self.width), itertools.repeat(point.y)
            ),
            map(point_factory, itertools.repeat(point.x), reversed(range(0, point.y))),
            map(
                point_factory,
                itertools.repeat(point.x),
                range(point.y + 1, self.height),
            ),
        )
    ...
    def is_visible(self, point: DiscretePoint2) -> bool:
        ...
        def test(p):
            return self.at(p) < this_height

        return any(map(lambda i: all(map(test, i)), self.generate_neighbors(point)))
    ...
    def get_scenic_score(self, point: DiscretePoint2) -> int:
        ...
        per_dim = map(
            ...
            map(lambda i: filter_scenic_direction(i), self.generate_neighbors(point)),
        )
