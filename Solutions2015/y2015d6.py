...
class Solution_2015_06(SolutionBase):
    ...
    def _bad_part_1_hook(self):
        ...
        for i in self.instructions:
            ...
            has_intersections = any(map(lambda x: bounds.intersects(x), pending_regions.keys()))

            if not has_intersections:
                assert not any(map(lambda pr: bounds.intersects(pr), pending_regions.keys()))
                print(f"ADD {bounds}")
                if i.operation == _Operation.TURN_ON:
                    pending_regions[bounds] = bounds.generate_bounded_points()  # type: ignore
            ...
            assert not any(map(lambda pr: bounds.intersects(pr), pending_regions.keys()))
            assert not any(map(lambda pr: full_bounds.intersects(pr), pending_regions.keys()))
            for a,b in itertools.combinations(pending_regions.keys(), 2):
                if a.intersects(b):
                    print("***", a)
                    print("***", b)
            ...
            if i.operation == _Operation.TURN_ON:
                s = itertools.chain(bounds.generate_bounded_points(), filter(lambda x: not bounds.contains(x), existing))
            elif i.operation == _Operation.TURN_OFF:
    ...
    def _part_1_hook(self) -> Optional[Answer_T]:
        ...
        all_bounds = reduce(lambda b,x: b.combine(x), map(lambda x: x.AABB, self.instructions))
        ...
        for i in reversed(self.instructions):
            print(f"{len(normal_points) + len(toggled_points):>10} Remain | {number_on:>9} definitely on| {len(normal_points):>9} points normal | {len(toggled_points):>9} toggled")
            if len(normal_points) == 0 and len(toggled_points) == 0:
                print("No more points, exiting early.")
                break

            s = set(filter(lambda x: x in normal_points or x in toggled_points, i.AABB.generate_bounded_points()))
            ...
            if i.operation == _Operation.TURN_OFF:
                ...
                toggled_points.symmetric_difference_update(s.intersection(normal_points))
