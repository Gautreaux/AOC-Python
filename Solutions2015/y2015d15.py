...
def generateAllCookies(
    ingredients: list[Ingredient], amt: int
) -> Generator[tuple[Recipe, int], None, None]:
    ...
    for i in range(0, amt + 1):
        remainder_amt = amt - i
        for (cap, dur, fla, tex), cal in generateAllCookies(
            remainder_ingredients, remainder_amt
        ):
            this_recipe = (
...
def generateNonZeroScores(
    recipe_iterable: Iterable[Recipe],
...
def teeMax(
    recipe_iterable: Iterable[Recipe], calorie_target: int = 500
) -> tuple[int, int]:
    max_any = 0
    max_cal = 0
