...
def check_date_code(date_code : DateCode) -> TestResult:
    ...
    return [TestResult.SOLUTION_INCORRECT, TestResult.ONE_STAR, TestResult.TWO_STAR][stars]
...
def test_all_days_multiprocessed(date_codes: list[DateCode] = list(genElapsedDateCodes())) -> dict[DateCode, TestResult]:
    ...
    for dc, result, elapsed_time_s in pool.imap_unordered(_check_date_code_wrapper, date_codes): 
        results.append((dc, result))

        elapsed_time_s = round(elapsed_time_s, 4)

        if result != TestResult.TWO_STAR or elapsed_time_s > 10:
            print(f"{dc.year:04}.{dc.day:02} - {elapsed_time_s:>010.4}s - {result.name}")
