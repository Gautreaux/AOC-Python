...
class CalibrationEquation:
    def __init__(self, target: int, operands: Iterable[int]):
        self.target: int = target
        self.operands: tuple[int, ...] = tuple(operands)

    def generate_valid_solutions(
        self, supported_operands: list[CalibrationEquationOperands]
    ) -> Iterator[str]:
...
def y2024d7(inputPath=None):
    ...
    part_2_operands = [
        CalibrationEquationOperands.ADD,
        CalibrationEquationOperands.MUL,
        CalibrationEquationOperands.CCAT,
    ]

    Part_1_Answer = sum(
        eq.target for eq in equations if eq.has_any_solution(part_1_operands)
    )
    Part_2_Answer = sum(
        eq.target for eq in equations if eq.has_any_solution(part_2_operands)
    )
