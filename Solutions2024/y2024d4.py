...
class WordSearch:
    ...
    def term_occurs_at_with_transformer(self, search_term: str, start: tuple[int, int], transformer: tuple[int, int]) -> bool:
    ...
    def get_occurances_starting_at(self, search_term: str, start: tuple[int, int]) -> tuple[tuple[int, int], ...]:
        ...
        transformers = tuple(p for p in product((-1, 0, 1), (-1, 0, 1)) if not all(t == 0 for t in p))
        ...
        results = tuple([t for t in transformers if self.term_occurs_at_with_transformer(search_term, start, t)])
        return results

    def get_occurances(self, search_term: str) -> dict[tuple[int, int], tuple[tuple[int, int]]]:
...
def _assert_sample_passes():
    ws_sample = WordSearch(_SAMPLE.splitlines())
    search_term = 'XMAS'
    ...
    for start, trans in expected_answers_start_and_transform.items():
        ...
        assert set(l) == set(trans), f'FAILED with: Start {start}, {len(trans)} expected but found {len(l)} {l}'


    assert sum(len(l) for l in expected_answers_start_and_transform.values()) == 18
    assert ws_sample.count_occurances('XMAS') == 18, str(ws_sample.count_occurances('XMAS'))
