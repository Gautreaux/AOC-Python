...
class DiskFile:
    ...
    @staticmethod
    def files_from_dense_map(densemap: str) -> list[DiskBlock]:
        files: list[DiskFile] = []
...
def compact_files(files: list[DiskFile]) -> list[DiskBlock]:
    ...
    blocks_to_place: deque[DiskBlock] = deque(chain.from_iterable(f.generate_blocks() for f in files))
...
def compact_files_smart(files: list[DiskFile]) -> list[DiskFile]:
    ...
    while to_move:
        ...
        for i in range(len(new_disk)-1):
            ...
            free_space = upper.start_location - (lower.start_location + lower.blocks_len)

            if free_space >= current.blocks_len:
                # print(f'Moving {current} between {lower} and {upper}')
                new_new_disk = new_disk[:i+1]
                new_new_disk.append(replace(current, start_location=(lower.start_location + lower.blocks_len)))
