...
class ClawMachine:
    ...
    def _is_valid_solution(self, a_presses: int, b_presses: int) -> bool:
        ...
        return (
            (((self.button_a_x * a_presses) + (self.button_b_x * b_presses)) == self.prize_x) and 
            (((self.button_a_y * a_presses) + (self.button_b_y * b_presses)) == self.prize_y)
        )
    ...
    def find_cheapest_solution(self, cost_per_a: int = 3, cost_per_b: int = 1) -> tuple[tuple[int, int], int] | None:
