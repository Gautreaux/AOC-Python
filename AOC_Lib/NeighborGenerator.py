...
def neighborGeneratorFactoryND(min_limits : List[int], max_limits: List[int], allow_diagonal:bool):
    ...
    def to_return(*args):
        ...
        for offsets in itertools.product((-1, 0, 1), repeat=n_dims):
            ...
            b = all(map(lambda x,l_b,u_b : x >= l_b and x <= u_b, new_pos, min_limits, max_limits))
