...
def getAdjacentHexTiles(
    position: Point2, gridOrientation: int
) -> Generator[Point2, None, None]:
    assert gridOrientation in [GRID_HORIZONTAL, GRID_VERTICAL]
    l = (
        _HEX_STEPS_HORIZONTAL
        if gridOrientation == GRID_HORIZONTAL
        else _HEX_STEPS_VERTICAL
    )
    for step in l:
...
def getAdjacentTiles(
    pos: HEX_POS_T, grid_orientation: int
) -> Generator[HEX_POS_T, None, None]:
    """Get all tiles adjacent to the pos"""
    assert grid_orientation in [GRID_HORIZONTAL, GRID_VERTICAL]
    n = (
        _HEX_STEPS_HORIZONTAL
        if (grid_orientation == GRID_HORIZONTAL)
        else _HEX_STEPS_VERTICAL
    )
...
def computeHexDistance(
    start_pos: HEX_POS_T, grid_dir: int, end_pos: HEX_POS_T = (0, 0)
) -> int:
