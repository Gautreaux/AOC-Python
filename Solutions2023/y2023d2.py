...
class Turn:
    ...
    def stasifies_limits(
        self, limit_red: int, limit_blue: int, limit_green: int
    ) -> bool:
        return (
            self.quantity_blue <= limit_blue
            and self.quantity_green <= limit_green
            and self.quantity_red <= limit_red
        )
...
class Game:
    ...
    def stasifies_limits(
        ...
        return all(
            t.stasifies_limits(limit_red, limit_blue, limit_green) for t in self.turns
        )
