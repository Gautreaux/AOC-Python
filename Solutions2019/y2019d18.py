...
def generateSuccessorStates(state, wallList):
    ...
    for transform in transforms:
        newState = MazeState(
            (0, 0), state.kList, d=state.depth + 1, doorList=state.dList
        )
...
def y2019d18(inputPath=None):
    ...
    def successorGenerator(x):
        return generateSuccessorStates(x, wallsList)
    ...
    for r in results:
        if r == None:
            print(
                "(Part 2) Failed to locate a goal for quadrant:" + str(results.index(r))
            )
        else:
            print(str(r.depth))
            dTotal += r.depth
