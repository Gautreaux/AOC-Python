...
def auto_determine_solution():
    ...
    rules: list[Rule] = [
        ...
        Rule(
            lambda i: i.A and i.B is False and i.D and i.E, Action.JUMP, "Greedy jump"
        ),
        ...
        Rule(
            lambda i: i.A and i.C is False and i.D and i.E,
            Action.JUMP,
            "Greedy Jump 2b",
        ),
        Rule(
            lambda i: i.A and i.B is False and i.D and i.E is False and i.H,
            Action.JUMP,
            "Must Jump",
        ),
        Rule(
            ...
            and i.F is False,
            Action.JUMP,
            "Jump when step would lead to trap",
        ),
        Rule(
            lambda i: i.A
            and i.B
        ...
        # Some other checks
        Rule(
            lambda i: i.D and i.E is False and i.H is False,
            Action.STEP,
            "Step if jump would trap",
        ),
        ...
        Rule(
            lambda i: i.A is False and i.D is False,
            Action.IGNORE,
            "Any Action Dies Next",
        ),
        Rule(
            lambda i: i.A is False and i.D and i.E is False and i.H is False,
            Action.IGNORE,
            "Any Action Dies Soon",
        ),
        Rule(
            lambda i: i.A
            and i.B is False
    ...
    assert all(
        map(lambda r: r.action in [Action.JUMP, Action.STEP], step_and_jump_rules)
    )
    ...
    if single_rules:
        print(
            f"The following {len(single_rules)} rules (of {len(step_and_jump_rules)}) are unique selectors:"
        )
...
class Solution_2019_21(IntcodeSolutionBase):
    ...
    def _run_spring_code(
        self, spring_code_program: SpringCodeProgram_T, execute_command: str
    ):
