...
def generateCornersForValue(pos: TestPos_t, size:int) -> Generator[TestPos_t, None, None]:
...
def _generateTestSquaresAcross(pos: TestPos_t, size:int) -> Generator[Generator[TestPos_t, None, None], None, None]:
...
def generateTestSetsAtResolution(size:int, min_x: int = 0, min_y:int=0) -> Generator[Generator[TestPos_t, None, None], None, None]:
    ...
    while True:
        ...
        all_gens.append(_generateTestSquaresAcross(pos=(min_x, next(y_vals)), size=size))


def applyTransform(positions: Iterable[TestPos_t], transform: tuple[int, int]) -> Iterable[TestPos_t]:
