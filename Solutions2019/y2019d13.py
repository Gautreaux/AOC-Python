...
def buildBlankGrid(
    init_value: Any = 0, x: int = _GRID_DIM_X, y: int = _GRID_DIM_Y
) -> list[list[Any]]:
    to_return = [None] * y
    for i in range(y):
...
class GameState:
    ...
    def getBallPos(self) -> tuple[int, int]:
        ...
        if len(self._ball_pos_candidate) == 1:
            ...
            raise RuntimeError(
                f"Total {len(self._ball_pos_candidate)} ball positions found"
            )
    ...
    def getPaddlePos(self) -> tuple[int, int]:
        ...
        if len(self._paddle_pos_candidate) == 1:
            ...
            raise RuntimeError(
                f"Total {len(self._paddle_pos_candidate)} paddle positions found"
            )
...
def tuplesFromOutput(
    ...
    if out_q.qsize() % 3 != 0:
        print(
            "[WARNING]: the size is {} and not a multiple of 3. Remaining items will be left.".format(
                out_q.qsize()
            )
        )
...
def runTillBlocked(
    io_event: asyncio.Event, run_task: asyncio.Task, loop: asyncio.AbstractEventLoop
):
